...
public class PageController {
     ...
     * @return JSON格式的操作结果
     */
    @PostMapping("/order/confirmReceipt") // 处理POST /order/confirmReceipt请求
    @org.springframework.web.bind.annotation.ResponseBody // 返回值直接作为HTTP响应体（JSON）
    public Map<String, Object> confirmReceipt(@RequestParam String orderId, HttpSession session) {
        Map<String, Object> result = new HashMap<>(); // 构建JSON响应
        Map<String, Object> user = (Map<String, Object>) session.getAttribute("user"); // 从Session获取用户信息
        if (user == null) { // 未登录
            result.put("success", false);
            result.put("message", "请先登录");
        ...
        try {
            ...
            HttpEntity<Void> entity = new HttpEntity<>(headers); // 封装请求实体
            // 调用订单服务的确认收货接口：POST /api/order/{orderId}/confirm
            ResponseEntity<String> response = restTemplate.exchange(
                "http://bookstore-order/api/order/" + orderId + "/confirm",
                HttpMethod.POST, entity, String.class); // 发起确认收货请求
            if (response.getStatusCode().is2xxSuccessful()) { // 请求成功
            ...
            result.put("success", false);
            result.put("message", "确认收货失败：" + e.getMessage()); // 返回异常信息用于调试
        }
        return result; // 返回JSON响应给前端AJAX
    }
    ...
    public String submitReview(@RequestParam String orderId,
        ...
        return "redirect:/order/detail?orderId=" + orderId; // 重定向到订单详情页，可查看评价结果
    }
