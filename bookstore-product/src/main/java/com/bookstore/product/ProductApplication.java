package com.bookstore.product;  // 声明当前类所在的包路径，属于商品服务模块
...
/**
 * 商品服务启动类
 * 这是商品微服务的入口，负责启动整个商品服务模块
 * 商品服务提供商品的增删改查、分类管理、搜索等功能
 *
...
@EnableFeignClients  // 开启Feign声明式HTTP客户端，允许通过接口方式调用其他微服务（如调用用户服务、订单服务等）
@MapperScan("com.bookstore.product.mapper")  // 自动扫描指定包下的MyBatis Mapper接口，将它们注册为Spring Bean
@ComponentScan(basePackages = {"com.bookstore.product", "com.bookstore.common"})  // 指定Spring扫描的包路径，包括商品模块和公共模块
public class ProductApplication {  // 商品服务的主类，程序从这里开始运行
    ...
    public static void main(String[] args) {  // Java程序入口方法，static表示无需创建实例即可调用
        SpringApplication.run(ProductApplication.class, args);  // 启动Spring Boot应用，传入主类的Class对象和命令行参数
    }
}
