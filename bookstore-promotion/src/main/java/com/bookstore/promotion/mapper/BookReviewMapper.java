 ...
 *   - insert()：插入记录（用户提交评价时使用）
 *   - deleteById()：根据ID删除记录（管理员删除评价时使用）
 *   - updateById()：根据ID更新记录（屏蔽、置顶、回复等操作时使用）
 *   - selectById()：根据ID查询记录
 *   - selectList()：查询列表
 *   - selectPage()：分页查询
 *   - 等等
...
public interface BookReviewMapper extends BaseMapper<BookReview> {  // 继承BaseMapper，泛型参数为BookReview实体类
}
