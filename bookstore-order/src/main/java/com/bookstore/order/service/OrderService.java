...
import com.bookstore.common.entity.CompensationRecord;  // 导入补偿记录实体
import com.bookstore.order.config.OrderConfig;  // 导入动态配置
import com.bookstore.order.mapper.CompensationRecordMapper;  // 导入补偿记录Mapper
import org.springframework.data.redis.core.StringRedisTemplate;  // 导入Redis模板（幂等性校验）

import java.security.MessageDigest;  // 导入消息摘要（用于生成幂等性Key的hash）
import java.security.NoSuchAlgorithmException;
import java.util.concurrent.TimeUnit;  // 导入时间单位（Redis TTL）
...
public class OrderService implements IOrderService {
    ...
    public OrderVO createOrder(String userId, OrderCreateDTO dto) {  // 创建订单的方法，接收用户ID和订单创建DTO
        ...
        // ==================== 动态配置守卫：可通过 Nacos 运行时关闭订单创建 ====================
        if (!orderConfig.isOrderCreationEnabled()) {
            throw new BusinessException(503, "系统维护中，暂时无法创建订单");
        }

        // ==================== 商品数量校验：防止超大订单 ====================
        if (dto.getItems() != null && dto.getItems().size() > orderConfig.getMaxOrderItems()) {
            throw new BusinessException(400, "单个订单最多包含 " + orderConfig.getMaxOrderItems() + " 个商品");
        }

        // ==================== 幂等性校验：防止重复提交（Redis SETNX） ====================
        // 原理：根据 userId + 商品列表 生成唯一 hash，作为幂等性 Key
        // 如果 Key 已存在，说明是重复提交（用户快速双击、网络重试等场景）
        String idempotentKey = IDEMPOTENT_KEY_PREFIX + userId + ":" + generateIdempotentHash(dto);
        boolean acquired = false;
        try {
            acquired = Boolean.TRUE.equals(
                    stringRedisTemplate.opsForValue().setIfAbsent(
                            idempotentKey, "1",
                            orderConfig.getIdempotencyTtlSeconds(), TimeUnit.SECONDS));
        } catch (Exception e) {
            log.warn("Redis 幂等性校验失败，降级放行: {}", e.getMessage());
            acquired = true;  // Redis 不可用时降级：允许通过，由后续业务逻辑保证正确性
        }
        if (!acquired) {
            throw new BusinessException(409, "请勿重复提交订单，请稍后再试");
        }
        // 创建订单实体对象，用于保存到数据库
        ...
        order.setStatus(OrderStatus.PENDING_PAYMENT.getStatus());  // 设置订单初始状态为OrderStatus.PENDING_PAYMENT.getStatus()
        ...
        // 【安全】信用卡号脱敏：只存储末四位，符合 PCI-DSS 合规要求
        String maskedCard = maskCreditCard(dto.getCreditCard());
        order.setCreditcard(maskedCard);  // 信用卡号（已脱敏，仅保留末四位）
        ...
        try {
            ...
            // ==================== 分布式事务补偿逻辑 + 持久化 ====================
            // 如果本地事务中任何步骤失败，但库存已通过Feign扣减，
            // 先尝试同步补偿，失败则持久化到 compensation_record 表由定时任务重试
            if (!deductedItems.isEmpty()) {
                log.error("创建订单失败，开始补偿已扣减的库存，订单ID: {}, 失败原因: {}",
                        order.getOrderid(), e.getMessage());
                for (String[] deducted : deductedItems) {
                    String productId = deducted[0];
                    int quantity = Integer.parseInt(deducted[1]);
                    try {
                        // 先尝试同步补偿（带重试）
                        compensateStockRestore(productId, quantity);
                    } catch (Exception ce) {
                        // 同步补偿也失败了，持久化到补偿记录表，由定时任务异步重试
                        CompensationRecord record = new CompensationRecord();
                        record.setOrderId(order.getOrderid());
                        record.setProductId(productId);
                        record.setQuantity(quantity);
                        record.setType("COMPENSATION");
                        record.setStatus("PENDING");
                        record.setRetryCount(0);
                        record.setMaxRetries(orderConfig.getStockCompensationMaxRetries());
                        record.setErrorMessage(ce.getMessage());
                        try {
                            compensationRecordMapper.insert(record);
                            log.info("补偿记录已持久化: orderId={}, productId={}, quantity={}",
                                    order.getOrderid(), productId, quantity);
                        } catch (Exception pe) {
                            log.error("【严重】补偿记录持久化也失败: orderId={}, productId={}", order.getOrderid(), productId, pe);
                        }
                    }
                }
            }
            ...
            // 重新抛出异常，让@Transactional回滚本地数据库操作
    ...
    /**
     * 订单支付
     * 将OrderStatus.PENDING_PAYMENT.getStatus()状态的订单更新为OrderStatus.PAID.getStatus()
     * @param userId 用户ID
     * @param orderId 订单ID
    ...
    public void payOrder(String userId, String orderId) {  // 订单支付方法
        ...
        if (!OrderStatus.PENDING_PAYMENT.getStatus().equals(order.getStatus())) {  // 检查订单状态是否为OrderStatus.PENDING_PAYMENT.getStatus()
            throw new BusinessException(400, "订单状态异常");  // 只有待支付的订单才能支付
        }
        order.setStatus(OrderStatus.PAID.getStatus());  // 更新订单状态为OrderStatus.PAID.getStatus()
    ...
    /**
     * 取消订单
     * 将OrderStatus.PENDING_PAYMENT.getStatus()状态的订单更新为OrderStatus.CANCELLED.getStatus()，并恢复商品库存
     * @param userId 用户ID
     * @param orderId 订单ID
     * @throws IllegalArgumentException 当订单不存在或状态不正确时抛出异常
     */
    ...
    public void cancelOrder(String userId, String orderId) {  // 取消订单方法
        ...
        order.setStatus(OrderStatus.CANCELLED.getStatus());  // 更新订单状态为OrderStatus.CANCELLED.getStatus()
        ordersMapper.updateById(order);  // 根据订单主键更新数据库记录
        ...
        for (OrderItem item : items) {  // 遍历每个订单项
            try {
                ...
                try {
                    ...
                    log.error("【严重】补偿记录持久化失败: orderId={}, productId={}", orderId, item.getProductId(), pe);
                }
            }
        }
    ...
    /**
     * 确认收货
     * 将OrderStatus.SHIPPED.getStatus()状态的订单更新为OrderStatus.COMPLETED.getStatus()
     * @param userId 用户ID
     * @param orderId 订单ID
     * @throws IllegalArgumentException 当订单不存在或状态不正确时抛出异常
     */
    ...
    public void confirmReceive(String userId, String orderId) {  // 确认收货方法
        ...
        if (!OrderStatus.SHIPPED.getStatus().equals(order.getStatus())) {  // 检查订单状态是否为OrderStatus.SHIPPED.getStatus()
            throw new BusinessException(400, "订单状态异常");  // 只有已发货的订单才能确认收货
        }
        order.setStatus(OrderStatus.COMPLETED.getStatus());  // 更新订单状态为OrderStatus.COMPLETED.getStatus()
        ordersMapper.updateById(order);  // 根据订单主键更新数据库记录
    }
    ...
    /**
     * 订单发货（管理员接口）
     * 将OrderStatus.PAID.getStatus()状态的订单更新为OrderStatus.SHIPPED.getStatus()
     * @param orderId 订单ID
     * @throws IllegalArgumentException 当订单不存在或状态不正确时抛出异常
     */
    @Transactional  // 开启事务
    public void shipOrder(String orderId) {  // 管理员发货方法
        ...
        if (!OrderStatus.PAID.getStatus().equals(order.getStatus())) {  // 检查订单状态是否为OrderStatus.PAID.getStatus()
            throw new BusinessException(400, "订单状态异常");  // 只有已支付的订单才能发货
        }
        order.setStatus(OrderStatus.SHIPPED.getStatus());  // 更新订单状态为OrderStatus.SHIPPED.getStatus()
        ordersMapper.updateById(order);  // 根据订单主键更新数据库记录
    }
    ...
    private OrderVO convertToVO(Orders order) {
        ...
        // Batch fetch product images to avoid N+1 Feign calls
        ...
        try {
            // 使用批量接口一次性获取所有商品信息，避免 N+1 远程调用
            String idsParam = String.join(",", productIds);
            Result<List<ProductVO>> batchResult = productFeignClient.batchGetProducts(idsParam);
            if (batchResult != null && batchResult.getData() != null) {
                for (ProductVO p : batchResult.getData()) {
                    imageMap.put(p.getId() != null ? p.getId() : p.getProductid(), p.getImageUrl());
                }
            }
        } catch (Exception e) {
            log.warn("批量获取商品图片失败，降级为逐个获取: {}", e.getMessage());
            // 降级：逐个获取
            for (String pid : productIds) {
                try {
                    Result<ProductVO> productResult = productFeignClient.getProductById(pid);
                    if (productResult != null && productResult.getData() != null) {
                        imageMap.put(pid, productResult.getData().getImageUrl());
                    }
                } catch (Exception ex) {
                    log.warn("获取商品图片失败, productId: {}", pid, ex);
                }
            }
        }
        ...
        vo.setItems(items.stream().map(item -> {
            ...
            itemVO.setImage(imageMap.get(item.getProductId()));
            return itemVO;
        }).collect(Collectors.toList()));
        return vo;
    }
    ...
    /**
     * 信用卡号脱敏处理
     * 只保留末四位，其余用 * 号替代，符合 PCI-DSS 合规要求
     * @param cardNumber 原始信用卡号
     * @return 脱敏后的卡号（如 ****1234）
     */
    ...
    /**
     * 带重试机制的库存补偿
     * 最多重试3次，每次指数退避（100ms、200ms、400ms）
     * 如果3次都失败，记录严重错误等待人工介入
     * @param productId 商品ID
     * @param quantity 需要恢复的数量
     */
    private void compensateStockRestore(String productId, int quantity) {
        int maxRetries = 3;
        for (int attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                productFeignClient.updateStock(productId, -quantity);
                log.info("库存补偿成功: productId={}, 恢复数量={}, 第{}次尝试", productId, quantity, attempt);
                return;
            } catch (Exception e) {
                if (attempt == maxRetries) {
                    log.error("【严重】库存补偿失败（已达最大重试次数），需要人工介入! productId={}, 需恢复数量={}, 错误: {}",
                            productId, quantity, e.getMessage(), e);
                    throw new RuntimeException("库存补偿失败: " + productId, e);
                } else {
                    log.warn("库存补偿第{}次失败，准备重试: productId={}, 错误: {}", attempt, productId, e.getMessage());
                    try {
                        Thread.sleep(100L * (1L << (attempt - 1))); // 指数退避: 100ms, 200ms, 400ms
    ...
    /**
     * 生成幂等性 Hash
     * 根据订单商品列表生成 MD5 摘要，相同商品组合产生相同 Hash
     * 这样同一用户提交相同商品组合的订单会被幂等性检查拦截
     *
     * @param dto 订单创建DTO
     * @return 16位 MD5 摘要字符串
     */
    private String generateIdempotentHash(OrderCreateDTO dto) {
        StringBuilder sb = new StringBuilder();
        if (dto.getItems() != null) {
            dto.getItems().stream()
                    .sorted((a, b) -> a.getProductId().compareTo(b.getProductId()))
                    .forEach(item -> sb.append(item.getProductId()).append(":").append(item.getQuantity()).append(","));
        }
        // 追加信用卡末四位作为用户操作区分（同商品不同支付方式视为不同订单）
        if (dto.getCreditCard() != null && dto.getCreditCard().length() >= 4) {
            sb.append("card:").append(dto.getCreditCard().substring(dto.getCreditCard().length() - 4));
        }
        try {
            MessageDigest md = MessageDigest.getInstance("MD5");
            byte[] digest = md.digest(sb.toString().getBytes(java.nio.charset.StandardCharsets.UTF_8));
            StringBuilder hex = new StringBuilder();
            for (int i = 0; i < 8 && i < digest.length; i++) {  // 取前8字节（16位hex），足够区分
    ...
    private void releaseIdempotentKey(String key) {
        try {
            ...
            log.warn("释放幂等性 Key 失败: key={}, error={}", key, e.getMessage());
            // 不抛出异常，Key 会在 TTL 后自动过期
        }
    }
}
